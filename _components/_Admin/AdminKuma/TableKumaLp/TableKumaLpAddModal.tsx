...
import { insertLps } from '@/_lib/supabase_lp';
...
export default function TableKumaLpAddModal() {
    // ============================
    // Atom管理
    // ============================
    // --- モーダル ---
    ...
    // --- プロダクト一覧 ---
    const [lps, setLps] = useAtom(lpsAtom);

    // Clinicsの作成 ============================
    const handleCreate = async () => {
        setIsProcessing(true);
        // --- 処理: 入力値を配列に変換 ---
        const newLps = formText.split(",").map((postKeyword) => ({category_name: "kuma", name: postKeyword.trim()}))
        console.log("newLps", newLps)

        // --- 処理: SelectPostを作成 ---
        try {
            // newKeywordsを一つずつ処理してproductsに追加
            const {data: resLp} = await insertLps(newLps);
            setLps(prevLps => [...prevLps, ...resLp??[]]);
            setAlertMessage("作成しました。"); // エラーメッセージを設定
            setAlertType("success");         // アラートタイプを 'success' に設定
            setShowAlert(true);              // アラートを表示
        } catch (error) {
            console.log(error);
            setAlertMessage("作成に失敗しました。"); // エラーメッセージを設定
                    ...
                    <div className="flex items-center justify-center min-h-screen md:block px-2 py-2 md:p-0 text-center">
                        ...
                        <div className="p-4 md:p-10 w-full md:w-3/5 h-full inline-block align-middle bg-slate-50 rounded-lg overflow-hidden transform border border-gray-300">
                            ...
                            <div className="mb-10 w-full overflow-auto text-left">
                                ...
                                <div className="flex justify-end text-xs text-gray-400">※カンマ区切りで入力すると複数作成できます。（例: kuma1, kuma2, ...）</div>
                            </div>
